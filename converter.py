...
for annotation in annotation_list:
    ...
    diff = int((img.shape[1] - img.shape[0]) / 2)

    for line in lines:
        new_line = line.strip('\n').split(',')
        bbox = (
            int(new_line[0]), int(new_line[1]), int(new_line[0]) + int(new_line[2]),
            int(new_line[1]) + int(new_line[3]))

    bbox = (max(bbox[0] - diff, 0), bbox[1], max(bbox[2] - diff, 0), bbox[3])
    if bbox[2] <= 0:
        continue
    if bbox[0] > img.shape[0] + diff:
        continue
